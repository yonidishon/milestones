...
visRoot = fullfile(diemDataRoot, 'vis_jump', 'train_v5_4');
    ...
    for i = 1:length(calcIdx)
        ...
        for ic = 1:nc
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after < videoLen) && (jumpFrames(ic) + after < length(gazeParam.gazeData)))
                ...
                [f, d, l, jumps] = jumpPairwiseFeatures5(srcFr, srcCands, dstFr, dstCands, options);
...
%                 [f, l, jumps] = jumpPairwiseFeatures(srcFr, srcPts, dstFr, dstPts, dstType, options);
    ...
    % save results
    save(fullfile(cache.featureRoot, '00_total.mat'), 'options', 'trainIdx', 'testIdx', 'precalcSubset', 'trainSubset', 'totalFeatures', 'posFeatures', 'negFeatures');
end
