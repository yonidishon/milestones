% run_jumpTrackTest4
% tests jump-track version 2 on frame range
...
visVideoRoot = fullfile(diemDataRoot, 'vis_jumptrack', 'RF_sbs_v3');
    ...
    while (frId <= nfr) % till we are inside frames range
        if (doJump) % jump
            ...
            if ((frames{i}(frId) - 3 > 1) && (frames{i}(frId) + cutAfter <= faces{iv}.length)) % can jump
                ...
                % rectangles to track
                [trackRect, trackRectScore] = candidate2Rect([n, m], jumps, evalOp.pThresh, rectSz');
                [sr, sri] = sort(trackRectScore, 'descend');
                trackRect = trackRect(:, sri(1:nTrackRect));
                trackRectScore = trackRectScore(:, sri(1:nTrackRect));
                
                fprintf('\tJump produced %d track rectangles, min score is %f\n', length(trackRectScore), min(trackRectScore));
...
%             rects = denseMap2Rects(predMaps(:,:,frId), predRectTh);
            ...
            for ir = 1:nr % for every rectangle
                ...
                fprintf('\tRectangle #%d: %d frames tracked\n', ir, trackLen(ir));
            end
