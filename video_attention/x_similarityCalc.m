% a modification to original similarityCalc to support the strange method
% of Fixation Bank
function [sim, extra] = x_similarityCalc(predMap, gaze, meas)
...
for im = 1:nmeas
    if (strcmpi(meas{im}, 'chisq')) % Chi square measure
        ...
        gaze.denseMap = imresize(gaze.denseMap,[15,20]);
        predMap = imresize(predMap,[15,20]);
    ...
    elseif (strcmpi(meas{im}, 'nss')); % Normalized Scanpath Saliency
        gaze.binaryMap = points2binaryMap(gaze.points, [20, 15]);
        sim(im) = mean(calcNSSscore(imresize(predMap,[15,20]), gaze.binaryMap));
    ...
    elseif (strcmpi(meas{im}, 'auc')); % Area Under Curve
        ...
        if (~any(gaze.binaryMap))
            ...
            [ac, R] = calcAUCscore(imresize(predMap,[15,20]), gaze.binaryMap, imresize(gaze.otherMap,[15,20])); % Shuffle AUC
