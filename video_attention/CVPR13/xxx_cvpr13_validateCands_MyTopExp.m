...
%             [dst_s{i}(ifr), h_s ,cand_hs,cand_hit_typ_s] = xxx_candDistGaze(gazeData.points{ifr}, filtcands{ifr});
            ...
            if isempty(cands{ifr});
                error('Top %d video %s frame %d no cands!!!',topnum,videos{iv},ifr);
            else
                hit{i}(ifr) = mean(h);
                presc{i}(ifr) = cand_h;
...
fid = fopen(fullfile(saveloc,sprintf('TopDima%d_data.txt',topnum)),'w');
for i = 1:nv
...
fprintf(fid,'##Dima (GBVS,OF) TopCand Num = %d Candidates##\n',topnum);
fprintf(fid,'\nMean distance: %fpx\n', dstAllMean);
fprintf(fid,'Mean hit rate: %f%%\n', hitAllMean * 100);
fprintf(fid,'Median hit rate: %f%%\n', median(hitAll) * 100);
fprintf(fid,'Mean precision: %f%%\n\n', prescAllMean * 100);
hit_AllMedian = median(hitAll);
save(fullfile(saveloc,sprintf('Dima_meansTop%d.mat',topnum)),'dstAllMean',...
                                                            'hitAllMean',...
                                                            'hit_AllMedian',...
                                                            'prescAllMean');
fclose(fid);
...
h = h / sum(h);
h = cumsum(h);
save(fullfile(saveloc,sprintf('DimacumhistTop%d.mat',topnum)),'x','h');
