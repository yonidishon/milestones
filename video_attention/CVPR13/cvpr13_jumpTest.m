%function visRoot = cvpr13_jumpTest(DataRoot)
% cvpr13_jumpTest
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% VERSIONS
...
testSubset = testSubset(testSubset>0);
nt = length(testSubset);
...
for i = 1:nt
    ...
    param = struct('videoReader', vr, 'depthVideoReader', dvr);
    ...
    %gazeData.points = gazeData.data;

    %gazeData.pointSigma = 10;
    ...
    try
        for ifr = 1:length(indFr)
            fr = preprocessFrames(param.videoReader, param.depthVideoReader, frames(indFr(ifr)), gbvsParam, ofParam, poseletModel, cache);
            ...
            [sim{i}(:,:,ifr), outMaps] = similarityFrame3(predMaps(:,:,indFr(ifr)), gazeData, measures, ...
                ...
                struct('method', 'saliency_RGBD', 'map', fr.saliencyRGBD), ...
