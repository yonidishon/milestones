function dstCands = xxx_jumpPerform6MyCand(vidname,srcCands, srcFrIdx, dstFrIdx, param, options, gbvsParam, ofParam, poseletModel, rf, cache)
...
dstFr = xxx_preprocessFramesPartial(param.videoReader, dstFrIdx, gbvsParam, ofParam, poseletModel);
...
dstCands = xxx_jumpCandidates3simpleMyCand(maps, options);
nDst = length(dstCands);

% features
features = xxx_jumpPairwiseFeatures6(srcFr, srcCands, dstFr, dstCands, options, cache); % v6, cached
