...
outRoot = fullfileCreate(diemDataRoot, 'cvpr13', 'demo_video');
...
% clips
compl = 1;
clips = cell(20 - compl, 1);
for i = 1:20-compl, clips{i}.testIdx = i+compl; end;
% clips = cell(4, 1);
...
load(fullfile(resRoot, '00_similarity.mat'), 'measures', 'methods', 'testIdx', 'testSubset');
...
for i = 1:nv
    ...
    try
        if (~isfield(clips{i}, 'frameIdx')) % take frames 30:end-30
            clips{i}.frameIdx = max(30,indFr(1)) : min(vr.numberOfFrames-30,indFr(end));
        end
