            ...
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after <= videoLen))
                ...
                elseif (strcmp(jumpFromType, 'prev-int')) % jump from previous integrated map
                    if (ic == 1 || isempty(cands{ic-1})) % first or empty previous
                        ...
                        [srcCands, ~, ~, ~] = xxx_sourceCandidatesMyMaps([],  predMaps(:,:,ic-1), options, sourceType);
                        %srcCands = xxx_sourceCandidatesMyMaps([], predMaps(:,:,ic-1), options, sourceType);
                    end
                else
                ...
                dstCands = xxx_jumpPerform6MyCand(videos{iv},srcCands, jumpFrames(ic)+before, jumpFrames(ic)+after, param, options, gbvsParam, ofParam, poseletModel, rf,cache); %TODO
