function [cands, posPts, posScore, negPts] = xxx_sourceCandidatesFull_Pm(fr, gazeMap, options, type)
...
if (isempty(gazeMap) || all(isnan(gazeMap(:))) || (min(gazeMap(:)) == max(gazeMap(:)))) % empty map
    ...
    elseif (strcmp(type, 'cand')) % use jump candidates
        ...
        elseif isfield(fr,'pcam')
            maps = cat(3,(ofx.^2 + ofy.^2),fr.saliency, fr.pcam);
        else
            maps = cat(3,(ofx.^2 + ofy.^2),fr.saliency, fr.pcas);
        end
        if fr.faces(1) == -1
            fr.faces=[];
        end
        cands = xxx_jumpCandidates3addAllPCAsPCAm(fr.faces, fr.poselet_hit,maps, options); %Yonatan - changed this line from jumpCnadidates to jumpCandidates3
