function [cands, posPts, posScore, negPts] = xxx_sourceCandidates_DimaRecon(fr, gazeMap, options, type)
...
if (isempty(gazeMap) || all(isnan(gazeMap(:))) || (min(gazeMap(:)) == max(gazeMap(:)))) % empty map
    ...
    elseif (strcmp(type, 'cand')) % use jump candidates
        ...
        if isempty(fr.faces) || fr.faces(1) == -1
            fr.faces=[];
        end
        [cands, ~, ~, ~] = jumpCandidates3(fr.faces, fr.poselet_hit, maps, options); %Yonatan - changed this line from jumpCnadidates to jumpCandidates3
