function [cands, posPts, posScore, negPts] = xxx_sourceCandidatesMyMaps(fr, gazeMap, options, type)
...
if (isempty(gazeMap) || all(isnan(gazeMap(:))) || (min(gazeMap(:)) == max(gazeMap(:)))) % empty map
    ...
    elseif (strcmp(type, 'cand')) % use jump candidates
        ...
        cands = xxx_jumpCandidates3simpleMyCand(maps, options); %Yonatan - changed this line from jumpCnadidates to jumpCandidates3
