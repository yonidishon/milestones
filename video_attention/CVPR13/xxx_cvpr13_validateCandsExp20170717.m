...
%GT candidates
cand_test_v6orig = '\\cgm47\Users\ydishon\Documents\Video_Saliency\DimaResults\jump_test_v6_orig';
...
gdrive = '\\cgm47\C\Users\ydishon\Documents\Video_Saliency\Dimarudoy_saliency\GDrive';
dropbox = '\\cgm47\C\Users\ydishon\Documents\Video_Saliency\Dimarudoy_saliency\Dropbox';
...
for kk=1:length(allowedtypes_cell)
...
cand_hits = [];
cand_hitss = [];
allowedtypes = allowedtypes_cell{kk};
for i = 1:nv
    iv = videoIdx(i);
    videoName = videos{iv};
    ...
    filtcands = xxx_filtcands(cands,allowedtypes);
    ...
    for ifr = 1:nfr
        ...
        if isempty(filtcands{ifr});
             hit_s{i}(ifr) = NaN;
             presc_s{i}(ifr) = NaN;
             fprintf('\nVideo %s : Fr#:%d , only semantic cands\n',videoName,s.frames(ifr));
        else
...
fprintf('##All available Candidates##\n');
fprintf('\nMean distance: %fpx\n', dstAllMean);
fprintf('Mean hit rate: %f%%\n', hitAllMean * 100);
fprintf('Median hit rate: %f%%\n', median(hitAll) * 100);
fprintf('Mean precision: %f%%\n\n', prescAllMean * 100);


fprintf('##%s Candidates##\n',strjoin(cand_types(allowedtypes),','));
fprintf('\nMean distance: %fpx\n', dst_sAllMean);
fprintf('Mean hit rate: %f%%\n', hit_sAllMean * 100);
fprintf('Median hit rate: %f%%\n', median(hit_sAll) * 100);
fprintf('Mean precision: %f%%\n\n', presc_sAllMean * 100);
...
figure('Name', 'Cumu Filtered Candidates');
