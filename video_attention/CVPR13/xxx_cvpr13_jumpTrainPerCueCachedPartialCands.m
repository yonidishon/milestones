...
VERSION = 'PartialCachedCands'; %TODO
diemDataRoot = '\\cgm47\D\DIEM';
uncVideoRoot = fullfile(diemDataRoot, 'video_unc');
gazeDataRoot = fullfile(diemDataRoot, 'gaze');
modelFile = fullfile(modelfeaturesaveloc, sprintf('00_trained_model_cached_v5_3_no_sem_%s.mat',VERSION));
...
%         end
        ...
        for ic = 1:nc
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after < videoLen) && (jumpFrames(ic) + after < length(gazeParam.gazeData)))
                % preprocess frames
                srcFr = xxx_preprocessFramesPartial(vr, jumpFrames(ic)+before, gbvsParam, ofParam, poseletModel, cache); %TODO
                dstFr = xxx_preprocessFramesPartial(vr, jumpFrames(ic)+after, gbvsParam, ofParam, poseletModel, cache); %TODO
                ...
                [srcCands, ~, ~, ~] = xxx_sourceCandidatesForTrainDimtry(srcFr, srcGazeMap, options, sourceType); %TODO
                ...
                [dstCands, dstPts, dstScore, dstType] = jumpCandidates3([],[], maps, options);
