function [features, distances, labels, jumps] = xxx_jumpPairwiseFeatures6PCAmPCAs(srcFr, srcCands, dstFr, dstCands, options, cache)
...
if (~isempty(cacheFile) && ~cache.renewFeatures && exist(cacheFile, 'file')) % load from cache
    ...
    for isrc = 1:nSrc
        for idst = 1:nDst
            features(:, nDst*(isrc-1)+idst) = xxx_jumpFeaturePoint6PCAmPCAs(srcFr, srcCands{isrc}, dstFr, dstCands{idst}, options);
