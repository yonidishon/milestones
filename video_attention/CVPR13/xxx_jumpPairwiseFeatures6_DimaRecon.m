function [features, distances, labels, jumps] = xxx_jumpPairwiseFeatures6_DimaRecon(srcFr, srcCands, dstFr, dstCands, options, cache)
...
if (exist('cache', 'var'))
    cacheD = fullfile(cache.featureRoot, dstFr.videoName);
    cacheFile = fullfile(cacheD, sprintf('%d__%d.mat', srcFr.index, dstFr.index));
else
    eror('xxx_jumpPairwiseFeatures6_DimaRecon:: no cache');
end
...
for isrc = 1:nSrc
    for idst = 1:nDst
        features(:, nDst*(isrc-1)+idst) = xxx_jumpFeaturePoint6Faces(srcFr, srcCands{isrc}, dstFr, dstCands{idst}, options);
