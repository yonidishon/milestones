...
%     cands = s.cands;
    ...
    GBVSOFcands = xxx_filtcands(s.dimacands,[4,5]);
    OFcands = xxx_filtcands(s.dimacands,[4]);
    GBVScands = xxx_filtcands(s.dimacands,[5]);
    % Dima cands

    %my cands to have middle
    s_my = load(strjoin(fullfile(mycands_loc(1),sprintf('%s.mat', videoName))),'cands');
    centercands = xxx_filtcands(s_my.cands,[1]);
...
%     cands = xxx_filtcands(s_my.cands,[5]);
    ...
    dst_OF{i} = zeros(nfr, 1);
    hit_OF{i} = zeros(nfr, 1);
    presc_OF{i} =  zeros(nfr, 1);
    for ifr = 1:nfr
        GBVSOFcurcands = GBVSOFcands{ifr};
        GBVScurcands = GBVScands{ifr};
        OFcurcands = OFcands{ifr};
        if length(GBVSOFcurcands)>topnums(topnum)
            [~,idx]=sort(cellfun(@(x)x.score,GBVSOFcurcands),'descend');
            GBVSOFcurcands = GBVSOFcurcands(idx(1:topnums(topnum)));
        end
        GBVSOFcurcands = [GBVSOFcurcands,centercands{ifr}];
        
        if length(GBVScurcands)>topnums(topnum)
            [~,idx]=sort(cellfun(@(x)x.score,GBVScurcands),'descend');
            GBVScurcands = GBVScurcands(idx(1:topnums(topnum)));
        end
        if length(OFcurcands)>topnums(topnum)
            [~,idx]=sort(cellfun(@(x)x.score,OFcurcands),'descend');
            OFcurcands = OFcurcands(idx(1:topnums(topnum)));
        end

        [dst{i}(ifr), h ,cand_h, cand_hit_typ] = xxx_candDistGaze(gazeData.points{ifr}, GBVSOFcurcands);
        [dst_s{i}(ifr), h_s ,cand_hs,cand_hit_typ_s] = xxx_candDistGaze(gazeData.points{ifr}, GBVScurcands);
        [dst_OF{i}(ifr), h_OF ,cand_OF,cand_hit_typ_OF] = xxx_candDistGaze(gazeData.points{ifr}, OFcurcands);
...
%         
        ...
        hit_OF{i}(ifr) = mean(h_OF);
        presc_OF{i}(ifr) = cand_OF;
        cand_hitsOF=[cand_hitsOF,cand_hit_typ_OF];

    end
    
    clear GBVSOFcands GBVScands OFcands;
...
dstMeanOF = zeros(nv ,1);
hitMeanOF = zeros(nv ,1);
precsMeanOF = zeros(nv,1);

fid = fopen(fullfile(saveloc,sprintf('TopCombinedOFGBVS%d_data.txt',topnums(topnum))),'w');
for i = 1:nv
    ...
    dstMeanOF(i) = mean(dst_OF{i}(~isnan(dst_OF{i})));
    hitMeanOF(i) = mean(hit_OF{i}(~isnan(hit_OF{i})));
    precsMeanOF(i) = mean(presc_OF{i}(~isnan(presc_OF{i})));
    fprintf(fid,'%s\t%fpx\t%f\t%f\n', videos{videoIdx(i)}, dstMeanOF(i), hitMeanOF(i),precsMeanOF(i));
end
...
dstAllOF = cell2mat(dst_OF);
dstAllOF = dstAllOF(~isnan(dstAllOF));
dstAllMeanOF = mean(dstAllOF);
hitAllOF = cell2mat(hit_OF);
hitAllOF = hitAllOF(~isnan(hitAllOF));
hitAllMeanOF = mean(hitAllOF);

prescAllOF = cell2mat(presc_OF);
prescAllOF = prescAllOF(~isnan(prescAllOF));
prescAllMeanOF = mean(prescAllOF);

fprintf(fid,'##GBVSOF Candidates##\n');
fprintf(fid,'\nMean distance: %fpx\n', dstAllMean);
...
fprintf(fid,'##GBVS Candidates##\n');
fprintf(fid,'\nMean distance: %fpx\n', dst_sAllMean);
fprintf(fid,'Mean hit rate: %f%%\n', hit_sAllMean * 100);
fprintf(fid,'Median hit rate: %f%%\n', median(hit_sAll) * 100);
fprintf(fid,'Mean precision: %f%%\n\n', presc_sAllMean * 100);

fprintf(fid,'##OF Candidates##\n');
fprintf(fid,'\nMean distance: %fpx\n', dstAllMeanOF);
fprintf(fid,'Mean hit rate: %f%%\n', hitAllMeanOF * 100);
fprintf(fid,'Median hit rate: %f%%\n', median(hitAllOF) * 100);
fprintf(fid,'Mean precision: %f%%\n\n', prescAllMeanOF * 100);
