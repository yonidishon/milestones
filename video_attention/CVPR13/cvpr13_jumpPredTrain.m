...
modelFile = fullfile(uncVideoRoot, '00_jumpPred_model_cvpr13_v1.mat');
...
cache.featureRoot = fullfileCreate(cache.root, '00_features_jump');
    ...
    for i = 1:length(calcIdx)
        ...
        for ic = 1:nc
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after < videoLen) && (jumpFrames(ic) + after < length(gazeParam.gazeData)))
                ...
                % features
                options.srcGroundTruth = srcGazeMap;
                options.dstGroundTruth = dstGazeMap;
                options.dstGroundTruthPts = gazeParam.gazeData{jumpFrames(ic)+after};
                [f, d, l] = jumpPredictorFeatures(srcFr, srcCands, dstFr, dstCands, options); % no cache
                clear srcFr dstFr;
