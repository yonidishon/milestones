...
VERSION = 'FullCachedCands_Pm+T_20171128'; % TODO
VERSION_FOR_model = 'FullCachedCands_Pm+T'; % TODO
...
cache.frameRoot = fullfile(diemDataRoot, 'cache'); % (YD)- 28/11/2017 changed to load from cache
cache.featureRoot = fullfileCreate(modelfeaturesaveloc,'SFU', sprintf('00_features_v6%s',VERSION_FOR_model));
...
% sim = cell(nt, 1);
% for ii = 1:nt
%     sim{ii} = load(fullfile(visRoot,sprintf('%s_similarity.mat',videos{ii})));
