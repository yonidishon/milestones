...
%     cands = s.cands;
    ...
    % my cands
    s_my = load(strjoin(fullfile(mycands_loc,sprintf('%s.mat', videoName))),'cands');
    filtcands = xxx_filtcands(s_my.cands,[4]);
    ...
    for ifr = 1:nfr
        curcand = cands{ifr};
        curcandfilt = filtcands{ifr};
        if length(curcand)>topnums(topnum)
            [~,idx]=sort(cellfun(@(x)x.candScore,curcand),'descend');
            curcand = curcand(idx(1:topnums(topnum)));
        end
        if length(curcandfilt)>topnums(topnum)
            [~,idx]=sort(cellfun(@(x)x.candScore,curcandfilt),'descend');
            curcandfilt = curcandfilt(idx(1:topnums(topnum)));
        end
        
        [dst{i}(ifr), h ,cand_h, cand_hit_typ] = xxx_candDistGaze(gazeData.points{ifr}, curcand);
        [dst_s{i}(ifr), h_s ,cand_hs,cand_hit_typ_s] = xxx_candDistGaze(gazeData.points{ifr}, curcandfilt);
...
fid = fopen(fullfile(saveloc,sprintf('TopOFGBVS%d_data.txt',topnums(topnum))),'w');
for i = 1:nv
