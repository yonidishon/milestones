...
cache.featureRoot = fullfileCreate(modelfeaturesaveloc, sprintf('00_features_v6%s',strcat(VERSION,'_verify_171201')));
...
%    else % calculate
        ...
        for ic = 1:nc
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after <= videoLen))
                ...
                dstCands = xxx_jumpPerform6(srcCands, jumpFrames(ic)+before, jumpFrames(ic)+after, param, options, gbvsParam, ofParam,poseletModel, rf,cache); %TODO
