...
%         end
        ...
        for ic = 1:nc
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after < videoLen) && (jumpFrames(ic) + after < length(gazeParam.gazeData)))
                ...
                [srcCands, ~, ~, ~] = xxx_sourceCandidatesMPCAAndGBVS(srcFr, srcGazeMap, options, sourceType); %TODO
                ...
                if (useDestGaze) 
                    maps = cat(3, dstFr.saliency, dstGazeMap);
                else
                    ...
                    maps = cat(3, dstFr.saliency, dstFr.pcam);
                end
                ...
                [f, d, l, jumps] = xxx_jumpPairwiseFeatures6PCAmGBVS(srcFr, srcCands, dstFr, dstCands, options, cache);
