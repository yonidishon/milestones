function dstCands = xxx_jumpPerform6(srcCands, srcFrIdx, dstFrIdx, param, options, gbvsParam, ofParam, poseletModel, rf, cache)
...
dstFr = xxx_preprocessFramesNoSaveToCache(param.videoReader, dstFrIdx, gbvsParam, ofParam, poseletModel, cache);

% destination candidates
maps = cat(3, (dstFr.ofx.^2 + dstFr.ofy.^2), dstFr.saliency);
if ~isempty(dstFr.faces) && dstFr.faces(1)<0
    dstFr.faces=[];
end
...
% features
features = xxx_jumpPairwiseFeatures6Faces(srcFr, srcCands, dstFr, dstCands, options, cache); % v6, cached
