...
fid = fopen(fullfile(saveloc,sprintf('TopGBVSPCAs%d_data.txt',topnum)),'w');
for i = 1:nv
...
fprintf(fid,'##Dima (GBVS,PCAS) TopCand Num = %d Candidates##\n',topnum);
fprintf(fid,'\nMean distance: %fpx\n', dstAllMean);
fprintf(fid,'Mean hit rate: %f%%\n', hitAllMean * 100);
fprintf(fid,'Median hit rate: %f%%\n', median(hitAll) * 100);
fprintf(fid,'Mean precision: %f%%\n\n', prescAllMean * 100);
hit_AllMedian = median(hitAll);
save(fullfile(saveloc,sprintf('GBVSPCAS_meansTop%d.mat',topnum)),'dstAllMean',...
                                                            'hitAllMean',...
                                                            'hit_AllMedian',...
                                                            'prescAllMean');
...
h = h / sum(h);
h = cumsum(h);
save(fullfile(saveloc,sprintf('GBVSPCAScumhistTop%d.mat',topnum)),'x','h');
