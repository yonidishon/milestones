...
%         end
        ...
        for ic = 1:nc
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after < videoLen) && (jumpFrames(ic) + after < length(gazeParam.gazeData)))
                ...
                [srcCands, ~, ~, ~] = sourceCandidates(srcFr, srcGazeMap, options, sourceType); %TODO
                ...
                [dstCands, dstPts, dstScore, dstType] = jumpCandidates3(dstFr.faces, dstFr.poselet_hit, maps, options);
