...
for k=1:length(all_set);
    ...
    pca_files = dir(fullfile('D:\head_pose_estimation\DIEMPCApng',movie_name_no_ext,'*.png'));
    ...
    for ii=1:(length(pca_files)/2)
        ...
        imwrite(sqrt(fr.ofx.^2+fr.ofy.^2),fullfile(dst_folder,movie_name_no_ext,sprintf('%06d_OFm.png',ii)),'BitDepth',16);
