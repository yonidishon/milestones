...
%         end
        ...
        for ic = 1:nc
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after < videoLen) && (jumpFrames(ic) + after < length(gazeParam.gazeData)))
                ...
                [srcCands, ~, ~, ~] = xxx_sourceCandidatesMy(srcFr, srcGazeMap, options, sourceType); %TODO
                ...
                [f, d, l, jumps] = xxx_jumpPairwiseFeatures6PCAmPCAs(srcFr, srcCands, dstFr, dstCands, options, cache);
