...
%   dvr             depth video stream as VideoReader object
...
for ifr = 1:nfr
    ...
    if (exist('cache', 'var') && exist(cacheFile, 'file') && ~cache.renew) % load from cache
        ...
        if (~isfield(s.data, 'index')) % add frame index
         ...
         %%%%%%%%%%%%%%%%%% GEORGE %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         if ~isfield(s.data, 'saliencyRGBD') % there is no RGBD saliency add it
%             rgb = double(data.image);
%             dep = read(dvr, ind);
%             if size(dep,3) > 1
%                 dep = dep(:,:,1);
%             end;
%             dep = imresize(dep, [s.data.height s.data.width]);
%             [h,w] = size(dep);
%             if (s.data.width ~= w)
%                 error('Dimensions of DEP and RGB image do not match: [%dx%d]~=[%dx%d]', s.data.height, s.data.width, h,w);
%             end;
%             dep = double(dep - min(dep(:))) + 1000;
%             data.saliencyRGBD = calc_rgbd_saliency(rgb, dep);
%             if  sum(sum(isnan(data.saliencyRGBD)))
%                 error('RGBD Saliency is NAN');
%             end;
%             save(cacheFile, 'data');
%         end
        ...
        data.saliencyGBVS = data.saliency;
         %%%%%%%%%%%%%%%%%% GEORGE %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         data.saliency = data.saliencyRGBD;
        clear s;
        frs{ifr} = data;
        continue;
    end
...
%         dep = dep(:,:,1);
%     end;
%     dep = imresize(dep, [vr.height vr.width]);
%     [h,w] = size(dep);
%     if (vr.width ~= w)
%         error('Dimensions of DEP and RGB image do not match: [%dx%d]~=[%dx%d]', vr.height, vr.width, h,w);
%     end;
%     dep = double(dep - min(dep(:))) + 1000;
%     data.saliencyRGBD = calc_rgbd_saliency(rgb, dep);
%     if  sum(sum(isnan(data.saliencyRGBD)))
