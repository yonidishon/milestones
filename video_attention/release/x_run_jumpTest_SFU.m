...
global dropbox;
global gdrive;
diemDataRoot = '\\cgm47\d\temp_DIEM_for_SFU_exp';
...
visRoot = fullfileCreate(diemDataRoot,'Results_SFU');
...
cache.gazeRoot =gazeDataRoot; % TODO
...
sfupng = '\\cgm47\d\head_pose_estimation\SFUpng';
videos = extractfield(dir(fullfile(sfupng)),'name');
...
for i = 3:nt %TODO
    ...
    sfubasepath = fullfile(sfupng,videos{iv});
    frames = extractfield(dir(fullfile(sfubasepath,'*.png')),'name');
    % prepare video
...
%         end
%     end
    dum = imread(fullfile(sfubasepath,frames{1}));
    [m,n,~]=size(dum);
    vr.Height = m;
    vr.Width = n;
    vr.numberOfFrames = numel(frames);
    videoLen =numel(frames);
    %param = struct('videoReader', vr);
    
    % load jump frames
    [jumpFrames, before, after] = jumpFramesLoad(diemDataRoot, iv, jumpType, 50, 1, videoLen);
    nc = length(jumpFrames);
    
    % load gaze data
    sfugazebasedir = '\\cgm10\Users\ydishon\Downloads\howManyBitsforSaliency\DATA\SEQ_SFU\';
    gazeData = x_GetFixationsSFU(sfugazebasedir,videos{iv},numel(frames),m,n);
%     s = load(fullfile(cache.gazeRoot, sprintf('%s.mat', videos{iv})));
%     gazeData = s.data;
%     clear s;
    gazePoints = cell(size(gazeData,3),1);
    for ii=1:length(gazePoints)
        [i,j]=find(gazeData(:,:,ii));
        gazePoints{ii}=[j,i];
    end
    gazeParam.gazeData = gazePoints;

    videoLen = min(videoLen, size(gazeData,3));
    ...
    if (~cache.renewJumps && exist(resFile, 'file')) % load from cache
        ...
        for ic = 1:nc
            tic
            data = yy_preprocessFrames(imread(fullfile(sfubasepath,frames{jumpFrames(ic)})), jumpFrames(ic),ofParam,sfubasepath,frames);
            %[m,s,c] = PCA_Saliency_all_color_Core(data.ofx,data.ofy,data.image);
%Dimtry's code
            param = struct('videoReader', data.image);
            if ((jumpFrames(ic) + before >= 3) && (jumpFrames(ic) + after <= videoLen))
                ...
                elseif (strcmp(jumpFromType, 'gaze')) % jump from gaze map
                    srcGazeMap = points2GaussMap(gazeParam.gazeData{jumpFrames(ic)+before}', ones(1, size(gazeParam.gazeData{jumpFrames(ic)+before}, 1)), 0, [n, m], 10); %TODO - CHECK 10 IS CORRECT
                ...
                dstCands = xx_jumpPerform6(srcCands, jumpFrames(ic)+before, jumpFrames(ic)+after, param, options, gbvsParam, ofParam, poseletModel, rf, cache,sfubasepath,frames);
