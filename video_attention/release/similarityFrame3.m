function [sim, outMaps, extra] = similarityFrame3(predMap, gaze, meas, varargin)
...
if (isfield(gaze, 'index') && iscell(gaze.points)) % gaze data is list - sample
...
outMaps(:,:,1) = predMap;
if (nargout > 2)
    [sim(1, :), extra.roc{1}] = similarityCalc(outMaps(:,:,1), gz, meas);
else
    sim(1, :) = similarityCalc(outMaps(:,:,1), gz, meas);
end
...
for imt = 1:nMethods
    ...
    elseif (strcmp(mname, 'center'))
        ...
        if (nargout > 2)
            [sim(1+imt, :), extra.roc{1+imt}] = similarityCalc(outMaps(:,:,1+imt), gz, meas);
        else
            sim(1+imt, :) = similarityCalc(outMaps(:,:,1+imt), gz, meas);
        end
