function [features, distances, labels, jumps] = x_jumpPairwiseFeatures6(srcFr, srcCands, dstFr, dstCands, options, cache)
...
if (~isempty(cacheFile) && ~cache.renewFeatures && exist(cacheFile, 'file')) % load from cache
    ...
    for isrc = 1:nSrc
        for idst = 1:nDst
            features(:, nDst*(isrc-1)+idst) = jumpFeaturePoint6(srcFr, srcCands{isrc}, dstFr, dstCands{idst}, options);
